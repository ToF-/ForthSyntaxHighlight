...
,T D2*    ,T D2/ ,T F+ ,T F- ,T F* ,T F/ ,T FNEGATE ,T FAB[ ,T FMAX ,T FMIN ,T FLOOR ,T FROUND ,T F** ,T FSQRT ,T FEXP ,T FEXPM1 ,T FLN ,T FLNP1 ,T FLOG ,T FALOG ,T FSIN ,T FCO[ ,T FSINCO[ ,T FTAN ,T FASIN ,T FACO[ ,T FATAN ,T FATAN2 ,T FSINH ,T FCOSH ,T FTANH ,T FASINH ,T FACOSH ,T FATANH ,T F2* ,T F2/ ,T 1/F ,T F~REL ,T F~AB[ ,T F~ ,T 0< ,T 0<= ,T 0<> ,T 0= ,T 0> ,T 0>= ,T < ,T <= ,T <> ,T = ,T > ,T >= ,T U< ,T U<= ,T U> ,T U>= ,T D0< ,T D0<= ,T D0<> ,T D0= ,T D0> ,T D0>= ,T D< ,T D<= ,T D<> ,T D= ,T D> ,T D>= ,T DU< ,T DU<= ,T DU> ,T DU>= ,T WITHIN ,T ?NEGATE ,T ?DNEGATE ,T TRUE ,T FALSE ,T . ,T U. ,T .R ,T U.R
ADDRESS-CATEGORY CATEGORY !
,T @ ,T ! ,T +! ,T C@ ,T C! ,T 2@ ,T 2! ,T F@ ,T F! ,T SF@ ,T SF! ,T DF@ ,T DF! ,T CHARS ,T CHAR+ ,T CELLS ,T CELL+ ,T CELL ,T ALIGN ,T ALIGNED ,T FLOATS ,T FLOAT+ ,T FLOAT ,T FALIGN ,T FALIGNED ,T SFLOATS ,T SFLOAT+ ,T SFALIGN ,T SFALIGNED ,T DFLOATS ,T DFLOAT+ ,T DFALIGN ,T DFALIGNED ,T MAXALIGN ,T MAXALIGNED ,T CFALIGN ,T CFALIGNED ,T ADDRESS-UNIT-BITS ,T ALLOT ,T ALLOCATE ,T HERE ,T MOVE ,T ERASE ,T CMOVE ,T CMOVE> ,T FILL ,T BLANK ,T UNUSED 
DEFINING-CATEGORY CATEGORY ! 
,T :         ,T CREATE    ,T VARIABLE  ,T CONSTANT  ,T 2VARIABLE ,T 2CONSTANT 
CONTROL-CATEGORY CATEGORY !
,T IF ,T ELSE ,T ENDIF ,T THEN ,T CASE ,T OF ,T ENDOF ,T ENDCASE ,T ?DUP-IF ,T ?DUP-0=-IF ,T AHEAD ,T CS-PICK ,T CS-ROLL ,T CATCH ,T THROW ,T WITHIN ,T BEGIN ,T WHILE ,T REPEAT ,T UNTIL ,T AGAIN ,T ?DO ,T LOOP ,T I ,T J ,T K ,T +DO ,T U+DO ,T -DO ,T U-DO ,T DO ,T +LOOP ,T -LOOP ,T UNLOOP ,T LEAVE ,T ?LEAVE ,T EXIT ,T DONE ,T FOR ,T NEXT ,T RECURSE 
STRING-CATEGORY CATEGORY !   \ <-- uh oh
,T S" ,T ." ,T ABORT"
COMMENT-CATEGORY CATEGORY !  \ <-- uh oh
,T S" (" L.

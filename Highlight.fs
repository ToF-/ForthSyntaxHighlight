...
: NORMAL
    HTML @ 0= IF ANSI-NORMAL 
    ELSE $CONTROL CATEGORY>COLOR HTML-COLOR THEN ;
...
: .SOURCE
    ...
    DEFINING OFF
    BEGIN
        STRING @ IF .RUN-STRING THEN
        COMMENT @ IF .RUN-COMMENT THEN
        LINE-COMMENT @ IF .RUN-LINE-COMMENT THEN
        .>TOKEN
        SWAP DUP WHILE
        TOKEN SWAP .TOKEN
        ?_EMIT
...
[IFUNDEF] TESTING 
S" STACK=993300" READ-COLOR-ARG
S" NUMBER=CC3300" READ-COLOR-ARG
S" STRING=009933" READ-COLOR-ARG
S" COMMENT=9900CC" READ-COLOR-ARG
S" DEFINING=006666" READ-COLOR-ARG
S" USERDEF=006699" READ-COLOR-ARG

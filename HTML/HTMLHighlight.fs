...
\ extract the rgb and font-weight from an attribut value
: VALUE>RGBW ( n -- col ? )
    DUP RGBW>COLOR 
...
\ display a string with the string color attributes
: STRING.
    $STRING ATTRIBUTES @
    VALUE>RGBW <SPAN>.
    SKIP-STRING. </SPAN>. ;

\ display a comment with the comment color attributes
: COMMENT.
    $COMMENT ATTRIBUTES @
    VALUE>RGBW <SPAN>.
    SKIP-COMMENT. </SPAN>. ;
...
HEX 00 00 00 RGB COLOR !
    ...
    CC 33 00 TRUE RGBW-VALUE $MEMORY   ATTRIBUTES !
    3D 3D 5C TRUE RGBW-VALUE $DEFAULT  ATTRIBUTES !
SOURCE.
BYE
[THEN]
